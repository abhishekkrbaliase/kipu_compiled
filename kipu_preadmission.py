...
def get_forms_from_tab(driver, href, form_name_list,
    ...
    """
    for covid-19 first check 'waived testing' tab followed by 'consent' tab
    this should be done only for MS & CC
    Vijay - check for which all date has to be checked and for which not to be checked
    """
    driver.get(href)
    ...
    for row in tab_data:
        if 'pre-admission assessment' in row['Document_Name__c'] or 'pa' in row['Document_Name__c']:
            if 'completed' in row['status']:
                ...
                mrn_data['open_forms'].append('pre-admission assessment')
        elif 'rtc' in row['Document_Name__c']:
            if 'completed' in row['status']:
                mrn_data['RTC_is_signed__c'] = 'Yes'
            else:
                mrn_data['open_forms'].append('rtc')
    ...
    for row in tab_data:
        if 'commitment to treatment' in row['Document_Name__c']:
            if 'signed' in row['status']:
                ...
                mrn_data['open_forms'].append('commitment to treatment')
    ...
    for row in tab_data:
        if 'orientation sign-off' in row['Document_Name__c'] or 'orientation checklist' in row['Document_Name__c']:
            if 'completed' in row['status']:
                ...
                if 'Staff' in sign_text or 'staff' in sign_text:
                    ...
                    mrn_data['Intake_Orientation_Date_Completed__c'] = to_date_no_timezone(
                        ...
                        # Abhishek Check Time is getting stripped - Done
                    )
            else:
                mrn_data['open_forms'].append('orientation sign-off')
            ...
            if 'completed' in row['status']:
                ...
                mrn_data['open_forms'].append('client personal items and valuables')
    ...
    for row in tab_data:
        ...
        if 'face to face' in row['Document_Name__c'] or 'f2f' in row['Document_Name__c']:
            if 'completed' in row['status']:
                ...
                mrn_data['open_forms'].append('f2f')
        elif (
            ...
            if 'completed' in row['status']:
                ...
                mrn_data['open_forms'].append('h&p')
            ...
            if 'completed' in row['status']:
                ...
                mrn_data['open_forms'].append('psychiatric eval')
    ...
    for row in tab_data:
        ...
        if 'biopsychosocial assessment' in row['Document_Name__c'] or 'bps' in row['Document_Name__c']:
            if 'completed' in row['status']:
                ...
                mrn_data['open_forms'].append('bps')
        ...
        if 'initial treatment plan' in row['Document_Name__c']:
            if 'completed' in row['status']:
                ...
                for sign_element in root_elements:
                    ...
                    elif 'Review' in sign_text or 'review' in sign_text:
                        sign_row_split = sign_text.split(",")
                ...
                # Abhishek - logic pending for ITP_target_date_was_updated__c - done
                """
                ...
                mrn_data['open_forms'].append('initial treatment plan')
        elif 'master problem list' in row['Document_Name__c']:
            if 'completed' in row['status'] or 'in use' in row['status']:
                driver.get(row['link'])
                root_elements = driver.find_elements(By.CLASS_NAME, 'border_top')
                for sign_element in root_elements:
                    sign_text = sign_element.text
                ...
                mrn_data['open_forms'].append('master problem list')
...
def prepare_df_for_salesforce(temp_df, list_columns):
    ...
    for x in list_columns:
        if not x in list(df.columns.values):
            df[x] = float('nan')
